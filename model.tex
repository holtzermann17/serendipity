...
\begin{tikzonimage}[width=.45\textwidth,angle=270]{figures/model-diagram/serendipity-attractor}%[tsx/show help lines]
% \draw [orange, line width=5pt] (1,1) circle [radius=0.75cm];
% \draw [orange, line width=5pt] (.5,.5) circle [radius=0.75cm];
\node (dynamic) at (.1, .47) {\emph{dynamic world}};
\node (trigger) at (.05, .26) {\textbf{trigger}};
\node (chance) at (.045, .165) {chance};
\node (bridge) at (.46, .84) {\textbf{bridge}};
\node (result) at (.05, .67) {\textbf{result}};
\node (result)[text width=2cm,align=center] at (.71, .7) {\textbf{prepared mind}};
\node (focus) at (.91, .79) {{\bf \textsc{focus shift}}};
\node (curiosity) at (.41,.55) {curiousity};
\node (sagacity) at (.75,.42) {sagacity};
\node (value) at (.04, .74) {value};
\node (influences)[text width=2cm,align=center,rotate=30] at (.98, .65) {{\small\baselineskip=2pt \emph{multiple influences}\par}};
\node (tasks)[text width=1.5cm,align=center,rotate=30] at (.86, .37) {{\small\baselineskip=2pt \emph{multiple tasks}\par}};
\node (contexts)[text width=1.5cm,align=center,rotate=30] at (.55, .43) {{\small\baselineskip=2pt \emph{multiple contexts}\par}};
\end{tikzonimage}
