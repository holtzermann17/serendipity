...
with \emph{low} serendipity: if the outcome was likely to happen, if
it happened by chance with little skill, or if the the value of the
result was low, then we would not say the outcome was serendipitous.
Campbell \citeyear{campbell} describes serendipity as ``the rational
exploitation of chance observation, especially in the discovery of
something useful or beneficial.''  We would be prepared to attribute
``minimal serendipity'' to cases where the observation was
\emph{moderately} likely, \emph{some} skill or effort was involved,
and the result was \emph{fairly good}.  For computational systems, if
most of the skill involved lies with the user, then there is little
reason to call the system's operation serendipitous -- even if it
consistently does its job very well.  For example, machines can
``learn'' to recognise instances of a given pattern quite
consistently, but it is typically an interesting surprise if a
