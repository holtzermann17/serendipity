...
In Campbell's \citeyear{campbell} description of serendipity as ``the
rational exploitation of chance observation, especially in the
discovery of something useful or beneficial'' one can see all the
...
and the result was only \emph{fairly good}.  For computational
...
consistently, but it is an interesting surprise if a computational
system independently finds an entirely new kind of pattern.
Furthermore, the position of the evaluator is important: a
