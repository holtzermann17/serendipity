...
\end{quote}
The same story formed part of the inspiration for Voltaire's \emph{Zadig}, and ``the method of Zadig'' was referenced in late 19th Century philosophy of science \cite{huxley1894science}.
...
firstly, we point out that work is involved in both phases of the process (even when chance plays a role), and secondly, we defer true ``novelty'' to the invention phase.
...
An active research community investigating computational models of serendipity exists in the field of information retrieval \cite{foster2003serendipity}, and specifically, in recommender systems \cite{Toms2000}. In the recommender system domain, \citeA{Herlocker2004} and \citeA{McNee2006} view serendipity as an important factor for user satisfaction, alongside accuracy and diversity.  Serendipity in recommendations is
...
or usefulness).  Rather, it the term is a description of a
certain kind of process.

Like us, Paul Andr{\'e} et al.~\citeyear{andre2009discovery}
propose a two-part model of serendipity encompassing ``the chance
encountering of information, and the sagacity to derive insight
from the encounter.''  According to Andr\'e et al., the first
...
to pursue this work by developing systems with better
representational features: \emph{domain expertise} and a
...
distinct step in the process that mental preparation can support,
...
investigated the feasibility of using designs of this sort in
multi-agent systems that learn by sharing and discussing partial
understandings
...
human designers \cite{Analogical-problem-evolution-DCC}.  The creation
of new patterns and higher-order analogies were considered in
Hofstadter and Mitchell's work on {\sf Copycat} and the subsequent
{\sf Metacat}, but these systems operate in a simple and fairly
...
to the absence of clear criteria (which something we address in
the current paper) but also due to sociological factors in
computing research.  Existing standards for assessing
computational creativity have historically focused on product
evaluations \cite{ritchie07}, and are increasingly complemented
...
\begin{quote}
``\emph{Tinkering is a process of serendipity-seeking that does not just tolerate uncertainty and ambiguity, it requires it. When conditions for it are right, the result is a snowballing effect where pleasant surprises lead to more pleasant surprises.}'' \cite{rao2015breaking}
