...
below, and revisit poetry flowcharts as a third example.  As Campbell
...
encountering a trigger is high.  The evolution of
...
very serendipitous.''  However, if individual threads in the search
...
According to the SPECS criteria, we are not required to progress to
Part C here, but for completeness, we note the following.  The {\sf
...
$\mathit{low}\times\mathit{low}\times\mathit{high}$, but the system
should be seen only as ``potentially serendipitous'' until there is a
